<template>
  <div class="filre">
    <ul class="top">
      <li class="topli">
        <div @click="tables" v-show='type'><img src="../../static/image/libi.png"
               alt=""></div>
    ...
    <div style="position:relative">
      <mt-loadmore :top-method="loadTop"
                   ref="loadmore">
        <div class="aaaaa">
          <div style="height:60px;line-height:60px;font-size: 0.4rem; font-family: 'PingFang-SC-regular';color: #303030;background:#f8f8f8;font-weight:bold;">
            <span><img src="../../static/image/sheb1.png"  style="width:0.6rem; vertical-align: middle; margin-bottom: 0.133333rem; margin-left: 0.4rem;"
                     alt=""></span><span style="display:inline-block;padding-left:0.3rem;font-weight:bold;">设备编号：&nbsp;&nbsp;</span>
            <span>{{snone}}</span>
          </div>
          ...
          <ul class="hello"
            ...
            <li>
              <span>设备状态：&nbsp;&nbsp;</span>
              <span style="float:right;">{{onlineone=="true"?"(在线"+hitName+")":"(离线"+hitName+")"}}</span>
              <span v-if='help==1'
                    class="regular"
                    style="float:right;"
              ...
              <!-- <span style="color: red;float: right;margin-right: 50px;">{{hitName}}</span> -->
            </li>
 ...
 -->
          ...
          <ul class="hello hellocopy">
            <li class="bottomchild topli"
                @click="starts">
              <span><img src="../../static/image/xit.png"
                     alt=""></span>
              <div class="helloDiv" style="margin-left: 0.2rem">系统信息</div>
          ...
          <ul class="hellos"
            ...
            <li>
              <span>水压报警上限阈值：&nbsp;&nbsp;</span>
              <span>{{con.devParaSnone}}Mpa</span>
            </li>
            <li>
              <span>水压报警下限阈值：&nbsp;&nbsp;</span>
              <span>{{con.devParaSntwo}}Mpa</span>
            </li>
          </ul>
          <ul class="bottom newbottom" v-show='type'>
          ...
          <ul class="bottom qibottom" v-show='type'
              >
         <!--    <li class="bottomoff lastlast" v-show='type'>
              <span class="fl"
            ...
            <li class="bottomoff lastlast">
              ...
              <button @click="mapchange">更新</button>
            ...
            <li class="bottomoff lastlast">
              ...
              <button @click="refreshaa"
                      style="background:#f6a839;">刷新</button>
            </li>
            <!-- <li class="bottomoff lastlast">
            ...
            <li class="bottomoff lastlast">
              ...
              <button @click="readmessages"
                      style="background:#4cd964;">读取</button>
            </li>
...
export default {
  ...
  methods: {
    ...
    waterValue(){
      ...
      axios({
        method:"post",
        url:that.httpUrl +'/devices/selectMinMax',
        data:Qs.stringify(data)
      }).then(function(message){
        console.log(message);
        that.con.devParaSnone=message.data.max;
        that.con.devParaSntwo=message.data.min;
      })
    },
    refreshyes () {
      ...
      this.$axios
        .post(
          ...
          this.$qs.stringify({
            ...
            cmd: 24,
            cmdValue: '11',
            remark:localStorage.getItem("weChatUsePhone"),
            type:'微信'
          })
        )
        .then(message => {
    ...
    abnormal (val) {
      console.log(val)
      if (val == 1) {
        this.$router.push({ path: '/historynum', query: { id: this.$route.query.id, lab: val ,sn:this.$route.query.sn} });
      } else {
        this.$router.push({ path: '/historynum', query: { id: this.$route.query.id ,sn:this.$route.query.sn} });
      }
      //
    },
    resultdata () {
      ...
      this.$axios
        ...
        .then(message => {
          console.log((message.data.i.devParaSn).substr(0,2))
      ...
      this.$axios
        .post(
          that.httpUrl + "Datapoints/getBatData",
        ...
        .then(message => {
          ...
          if (message.data.resCode == 0) {
            ...
            for (let i = 0; i < messData.length; i++) {
              ...
              switch (messData[i].id) {
                ...
                case '1004': that.con.devSoftVerison = messData[i].value
                  break;
                case '2005': that.con.waterPressure = (messData[i].value) / 10
                  break;
            ...
            if (that.con.devWarning == 0||that.con.devWarning ==64 ||that.con.devWarning ==16) {
              ...
              let powNoce='';
              let fault_hit='';
              powNoce=parseInt(that.con.devWarning,10).toString(2);
              console.log(that.con.devWarning)
              console.log(powNoce)
              ...
              fault_hit=powNoce.substring(8,9);
              console.log(powNoce)
              if(fault_hit=='1'){
                that.hitName="，非法开启"
              }else{
    ...
    aifa () {
      ...
      that.$axios
        ...
        .then(message => {
              ...
              that.$axios
                ...
                .then(message => {
                  if (message.data.resCode == 0) {
                    ...
                    for (let i = 0; i < messData.length; i++) {
                      switch (messData[i].id) {
                        case '2010': that.threeVlaue = messData[i].value
                          break;
                        default: break;
                      }
                    }
          ...
          //           .then(res => {
          //               console.log(res)
          //               if(JSON.parse(( message.data.errMsg).slice(4,res.data.errMsg.length)).data.status==4){
          //                 that.value13=true;
          //               that.success=1;
          //                that.ttt=true;
    ...
    glisplab () {
      ...
      //                 .then(res => {
      //                     console.log(res)
      //                     if(JSON.parse(( message.data.errMsg).slice(4, message.data.errMsg.length)).data.status==4){
      //                     that.value13=false;
      //                     that.ttt=true;
      //                     that.success=8;
      //                       clearTimeout(t);
      ...
      //                       that.colsp2=0
      //                       that.con.devDefence='未布防'
      //                     }else if(JSON.parse(( message.data.errMsg).slice(4, message.data.errMsg.length)).data.status==0){
      //                       that.value13=true;
      //                       that.tit=true;
      //                       that.loser=86
      //                       Indicator.close();
              ...
              that.$axios
                ...
                .then(message => {
                  if (message.data.resCode == 0) {
                    ...
                    for (let i = 0; i < messData.length; i++) {
                      switch (messData[i].id) {
                        case '2008': that.twoValue = messData[i].value
                          break;
                        default: break;
                      }
                    }
              ...
              that.$axios
                ...
                .then(message => {
                  if (message.data.resCode == 0) {
                    ...
                    for (let i = 0; i < messData.length; i++) {
                      switch (messData[i].id) {
                        case '2012': that.oneValue = messData[i].value
                          break;
                        default: break;
                      }
                    }
                                                        ...
                                                        .then(message => {
                                                          if (message.data.resCode == 0) {
                                                            ...
                                                            if (that.oneValue != 18) {
                                                              ...
                                                              that.colsp1 = 1;
                                                              that.con.devControl = '已激活';
                                                              that.getTimeToDevice(that.$route.query.id)
                                                            }
                                                          } else {
                                                            clearTimeout(settimeafours)
                                                            that.value21 = true;
    ...
    getTimeToDevice(dev){
      this.$axios.post(
        this.httpUrl+"/devices/updateActivationTime",
        this.$qs.stringify({
          deviceId:dev,
