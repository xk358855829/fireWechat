<template>
  <div class="su">
    ...
    <div class="pos"
      ...
      <div class="last" @click="nextClick('white')">
    ...
    <div class="title">
      <div class="titleT">上报时间</div>
      <div class="titleV">水压值</div>
    </div>
    <div class="statistics">
      <div class="mainBody" v-for="item in mesages">
        <div class="wtimes">{{item.at}}</div>
        <div class="wvalue">{{setwater(item.value)}}</div>
      </div>
      <!-- <div id="bar"></div> -->
    </div>
...
export default {
  ...
  mounted: function () {
    ...
    this.htime = this.common.getNowFormatDate3().slice(0, 10)
    ...
    that.$axios
      ...
      .then(res => {
        ...
        that.mesages=mesg.data.datastreams[0].datapoints.reverse()
        console.log(mesg);
        if(mesg.errno==0){
          console.log(11)
  ...
  methods: {
    setwater(value){
      ...
      if(val<0||val>160){
        ...
        if (val >= 0 || val <= 30) {
          return ((val + 8) / 100).toFixed(2) + 'Mpa'
        } else if (val > 30 || val <= 60) {
          return ((val + 10) / 100).toFixed(2) + 'Mpa'
        } else if (val > 60 || val <= 90) {
          return ((val + 12) / 100).toFixed(2) + 'Mpa'
        } else if (val > 90 || val <= 159) {
          return ((val + 15) / 100).toFixed(2) + 'Mpa'
        }
      }
    },
    handelChange1 (val) {
